...
public static class ProductEnpoints
{
    public static void MapProductEnpoints(this WebApplication app)
    {
        ...
        app.MapGet("/api/history", GetProductsAsync);
    }
    ...
    internal static async Task<IResult> GetProductsAsync(IProductRepository repository, IMapper mapper, string filter)
    {
        try
        {
            var products = await repository.GetProductsAsync(filter);
            return products is not null ? Results.Ok(products.Select(x => mapper.Map<ProductOutputDto>(x))) : Results.BadRequest();
        }
        catch (Exception e)
        {
            return Results.BadRequest(e.Message);
